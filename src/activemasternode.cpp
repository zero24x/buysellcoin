...
//
// Bootup the masternode, look for a 4000 BUYSELLCOIN input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in Buysellcoin.conf */
...
{
    CBuysellcoinAddress address(collateralAddress);

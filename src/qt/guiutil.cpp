...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Buysellcoin"))
        return false;
...
{
    // Convert Buysellcoin:// to Buysellcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Buysellcoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Buysellcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Buysellcoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Buysellcoin.lnk
...
{
    return GetAutostartDir() / "Buysellcoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Buysellcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Buysellcoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Buysellcoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Buysellcoin app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Buysellcoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Buysellcoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Buysellcoin-Qt"));

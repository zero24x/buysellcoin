...
{
    // Convert Buysell:// to Buysell:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Buysell://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Buysell:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Buysell.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Buysell.lnk
...
{
    return GetAutostartDir() / "Buysell.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Buysell.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Buysell\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Buysell app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Buysell app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Buysell-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Buysell-qt [" + tr("command-line options") + "]                     " + "\n";

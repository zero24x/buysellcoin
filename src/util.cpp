...
int nAnonymizeBuysellcoinAmount = 1000;
...
#else
    const char* pszModule = "Buysellcoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Buysellcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Buysellcoin
    // Mac: ~/Library/Application Support/Buysellcoin
    // Unix: ~/.Buysellcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Buysellcoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Buysellcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "buysellcoind.pid"));
